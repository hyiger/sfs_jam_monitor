...
def main() -> int:
    ...
    ap.add_argument("--log-json", action="store_true", help="Emit logs as JSON lines (one JSON object per line)")
    ...
    # Additional flags used in code paths
    ap.add_argument("--runout-test", action="store_true")
    ...
    def _fields(now: Optional[float] = None) -> dict:
        ...
        return {
            ...
            "armed": armed,
            "serial_connected": link.connected.is_set() if 'link' in globals() or 'link' in locals() else False,
    ...
    if args.runout_test:
        ...
        log.info(
            "RUNOUT TEST: GPIO %d (active-%s). Toggle filament present/out to observe changes.",
            args.runout_gpio,
            "low" if args.runout_active_low else "high",
        )
    ...
    def trigger(reason: str):
        ...
        if reason == "runout":
            ...
            st.jam_count += 1
            log.info(
                "JAM: no pulses for %.2fs (sending action=%s)",
                time.time() - st.last_pulse_time,
    ...
    if runout_pin is not None:
        ...
        def on_runout_change():
            ...
            def deb(edge: float):
                ...
                with lock:
                    if st.last_runout_edge_time != edge:
                        return
    ...
    try:
        while True:
            ...
            if args.printer_heartbeat_seconds > 0 and link.connected.is_set() and (now - last_phb) >= args.printer_heartbeat_seconds:
                ...
                armed = (now <= st.armed_until) and st.ever_pulsed and (not st.latched)
                announce(
                    f"SFS: OK enabled={int(st.enabled)} armed={int(armed)} latched={int(st.latched)} reason={st.reason or 'none'} pulses={st.pulse_total} runout={int(st.runout_asserted)}"
                )
            ...
            if st.latched:
                continue
            if now < st.grace_until or now < st.post_trigger_grace_until:
                continue
