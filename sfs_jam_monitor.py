...
def main() -> int:
    ...
    ap.add_argument("-p", "--port", default="", help="Printer serial port (required unless --doctor)")
    ...
    if args.version:
        print(f"sfs-jam-monitor {VERSION}")
        return 0
    ...
    if args.doctor:
        print("Doctor mode: GPIO wiring sanity check (no serial, no pause)")
        print(f" - MOTION GPIO (BCM): {args.motion_gpio}")
        print(f" - RUNOUT GPIO (BCM): {args.runout_gpio}")
        print("Instructions:")
        print("  1) With filament installed, RUNOUT should typically be not asserted.")
        print("  2) Remove filament to toggle RUNOUT and confirm it changes.")
        print("  3) Move filament through the SFS to generate MOTION pulses.")
        print("Press Ctrl-C to exit.")
        print("")

        motion_pin = DigitalInputDevice(args.motion_gpio, pull_up=True, bounce_time=0.001)
        runout_pin = DigitalInputDevice(args.runout_gpio, pull_up=True, bounce_time=0.01)
        ...
        motion_pin.when_activated = on_pulse

        try:
            while True:
                runout_asserted = not runout_pin.is_active  # active-low
                if last_runout is None or runout_asserted != last_runout:
                    last_runout = runout_asserted
                    print(f"[DOCTOR] RUNOUT asserted={runout_asserted}")

                now = time.time()
                if (now - last_print) >= 1.0:
                    last_print = now
                    print(f"[DOCTOR] MOTION pulses total={pulse_count[0]}")

                time.sleep(0.05)
        except KeyboardInterrupt:
            print("")
            print("Doctor mode stopped.")
        finally:
            try:
                motion_pin.close()
                runout_pin.close()
    ...
    if not args.port:
        ap.error("--port is required unless --doctor is used")
